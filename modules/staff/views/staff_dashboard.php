...
// Patient + Doctor ge nama (assigned_doctor = Doctor.user_id)
$baseSql = "SELECT p.*,
                   CONCAT(p.first_name, ' ', p.last_name) AS full_name,
                   CONCAT('Dr. ', d.first_name, ' ', d.last_name) AS doctor_name
            FROM Patient p
            LEFT JOIN Doctor d ON d.user_id = p.assigned_doctor";

if ($search !== '') {
    $stmt = $conn->prepare(
        $baseSql . "
         WHERE CONCAT(p.first_name, ' ', p.last_name) LIKE CONCAT('%', ?, '%')
    ...
    $result = $conn->query($baseSql . " ORDER BY p.first_name ASC, p.last_name ASC");
...
// Update card eke doctor dropdown ekata doctors load karanna
$doctors = [];
try {
    $dres = $conn->query("SELECT user_id, first_name, last_name FROM Doctor ORDER BY first_name, last_name");
        ...
        <div class="content">
            ...
            <?php elseif ($msg === 'error'): ?>
                ...
                <div class="alert alert-err">
                    <?php echo $reason !== '' ? e($reason) : 'Something went wrong. Please try again.'; ?>
                </div>
                    ...
                    <table>
                        ...
                        <?php if ($result && $result->num_rows > 0): ?>
                            ...
                            <?php while ($row = $result->fetch_assoc()): ?>
                                ...
                                foreach (preg_split('/\s+/', trim($fullName)) as $w) {
                                    if ($w !== '') {
                                        $initials .= strtoupper(mb_substr($w, 0, 1));
                                    }
                                }

                                $avatar = mb_substr($initials, 0, 2);

                                // Patient ge id eka = user_id
                                $patientId = $row['user_id'] ?? '';

                                // Doctor ge nama (Doctor table eken)
                                $doctorVal = $row['doctor_name'] ?? '';

                                $status = $row['status'] ?? '';

                                $statusCls = strtolower(
                                ...
                                <tr>
                                    ...
                                    <!-- STATUS -->
                                    <td>
                                        <span class="status-pill <?php echo e($statusCls); ?>">
                                            <?php echo e($status); ?>
                                        </span>
                                    </td>
                            ...
                            <tr>
                                ...
                                >
                                    ...
                                    <?php if ($search !== ''): ?>
                                        ...
                                        No patient found with full name containing
            ...
            <!-- PATIENT DETAILS CARD (Update / Delete -> medical_actions.php) -->
            ...
            >
                ...
                <div class="form-card-header">
                    ...
                    <div>
                        <h2 id="cardTitle">Patient Details</h2>
                        <p id="cardSub"></p>
                    </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="fFirst" name="first_name" class="form-input" required>
                    </div>

                    <div class="form-group">
                        <label>Last Name</label>
                        <input type="text" id="fLast" name="last_name" class="form-input" required>
                    </div>

                    <div class="form-group">
                        <label>Age</label>
                        <input type="number" id="fAge" name="age" class="form-input" min="0" max="120" required>
                    </div>

                    <div class="form-group">
                        <label>Cancer Type</label>
                        <select id="fCancer" name="cancer_type" class="form-input" required>
                    ...
                    <div class="form-group">
                        ...
                        <select id="fStage" name="stage" class="form-input" required>
                            <option value="">Select</option>
                    ...
                    <div class="form-group">
                        <label>Status</label>
                        <select id="fStatus" name="status" class="form-input" required>
                    ...
                    <div class="form-group">
                        ...
                        <select id="fDoctor" name="assigned_doctor" class="form-input" required>
                            <option value="">Select Doctor</option>
                            <?php foreach ($doctors as $d): ?>
                                <option value="<?php echo (int)$d['user_id']; ?>">
                                    Dr. <?php echo e($d['first_name'] . ' ' . $d['last_name']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <!-- CARD ACTION BUTTONS -->
                <!-- Update button eka DOM eke mulin thiyenne, Enter ebuwama Delete wenna epa. Pennana order eka CSS "order" walin -->

                <div
                    style="display:flex; justify-content:flex-end; gap:10px; padding:24px 0 8px;"
                >
                    ...
                    <button
                        type="button"
                        onclick="closeCard()"
                        style="order:1; padding:10px 20px; background:#fff; color:#334155; border:1px solid #cbd5e1; border-radius:8px; cursor:pointer; font-weight:600;"
                    >
                        Close
                    </button>
                    ...
                    <button
                        ...
                        style="order:3; padding:10px 20px; background:#2563eb; color:#fff; border:none; border-radius:8px; cursor:pointer; font-weight:600;"
                    >
                        Update
                    </button>
                    ...
                    <button
                        ...
                        style="order:2; padding:10px 20px; background:#dc2626; color:#fff; border:none; border-radius:8px; cursor:pointer; font-weight:600;"
                    >
                        Delete
                    </button>
...
<script>
    ...
    function openCard(btn)
    {
        ...
        document.getElementById('cardTitle').innerText = p.full_name;
